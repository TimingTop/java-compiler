...
public class DfaConstructor {
    ...
    //使用二维数组表示DFA有限状态自动机
    private final int[][] dfaStateTransformTable = new int[MAX_DFA_STATE_COUNT][ASCII_COUNT + 1];
    ...
    public int[][] convertNfaToDfa() {
        ...
        input.add(nfaMachine.startNode);// 拿到 NFA
        Set<Nfa> nfaStartClosure = nfaIntepretor.e_closure(input); // 求闭包
        Dfa start = Dfa.getDfaFromNfaSet(nfaStartClosure); // 闭包就是一个 DFA 节点
        dfaList.add(start);
        ...
        while (currentDfaIndex < dfaList.size()) {
            ...
            //下面代码用于调试演示
            boolean debug = currentDfa.stateNum == 5 || currentDfa.stateNum == 7;
            //上面代码用于调试演示

            for (char c = 0; c <= ASCII_COUNT; c++) {
                Set<Nfa> move = nfaIntepretor.move(currentDfa.nfaStates, c);// 对于每个输入，DFA 节点中
                // 多个 NFA 的跳转是什么
