...
public class MinimizeDFA {
    ...
    private final DfaGroupManager groupManager = new DfaGroupManager();
    int[][] minDfa = null;
    ...
    private boolean doGroupSeprationOnInput(DfaGroup group, Dfa first, Dfa next, char c) {
        ...
        if (groupManager.getContainingGroup(goto_first) != groupManager.getContainingGroup(goto_next)) { // 看看 dfa 是不是在同一组
            if (newGroup == null) {
