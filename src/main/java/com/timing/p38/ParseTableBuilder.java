...
public class ParseTableBuilder {
    ...
    private final HashMap<Integer, Symbols> symbolMap = new HashMap<Integer, Symbols>();
    private final ArrayList<Symbols> symbolArray = new ArrayList<Symbols>();
    ...
    private void addSymbolFollowSet(Symbols symbol) {
        if (isSymbolTerminals(symbol.value) == true) { // 终结符没有  followSet
            return;
        }
        ...
        for (int i = 0; i < symbol.productions.size(); i++) {
             ...
             * follow(a2) is union of first(a3) first(a4) ... first(an) first(b)
             * ...
             */
            for (int j = 0; j < rightSize.length; j++) {
                Symbols current = symbolMap.get(rightSize[j]); // a1
                if (isSymbolTerminals(current.value) == true) {
                    continue;
                }

                for (int k = j + 1; k < rightSize.length; k++) { // a2+a3+...
             ...
             */
            int pos = rightSize.length - 1;   // a
            while (pos >= 0) {
                Symbols current = symbolMap.get(rightSize[pos]);
                if (isSymbolTerminals(current.value) == false) {
                    addSetToFollowSet(current, symbol.followSet);  // follow(a) include follow(s)
                }
